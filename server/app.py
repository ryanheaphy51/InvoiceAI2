    ...
    if not DATA_PATH.exists():
        ...
        DATA_PATH.write_text(json.dumps({
            ...
            "settings": {
                "companyName": "",
                "logoUrl": "",
                "defaultPaymentTerms": "On receipt",
                "currency": "gbp"
            }
        }, indent=2))
...
def compute_invoice_totals(invoice):
    ...
    for item in invoice.get("items", []):
        ...
        unit_cost = float(item.get("sellingPrice", item.get("cost", 0)))
...
def send_invoice_email(customer, invoice, settings):
    ...
    <html>
      <body style='font-family: Arial, sans-serif;'>
        ...
        <p><strong>Total:</strong> {invoice['total']} {invoice.get('currency', 'gbp').upper()}</p>
        <p><strong>Due Date:</strong> {invoice.get('dueDate', 'N/A')}</p>
        <p>Payment Terms: {invoice.get('paymentTerms', settings.get('defaultPaymentTerms', 'On receipt'))}</p>
      </body>
    </html>
    """
...
def build_dashboard(data):
    ...
    return {
        "monthlyPayments": monthly_points,
        "topProducts": top_products[:8],
        "snapshot": {
            "totalInvoices": len(data.get("invoices", [])),
            "totalCustomers": len(data.get("customers", [])),
            "paymentsThisMonth": round(payments_this_month, 2)
        }
    }
        ...
        try:
            ...
            if path == "/api/products":
                if method == "GET":
                    ...
                    product = {
                        ...
                        "cost": float(payload.get("costPrice", payload.get("cost", 0))),
                        "costPrice": float(payload.get("costPrice", payload.get("cost", 0))),
                        "salesPrice": float(payload.get("salesPrice", payload.get("rrp", 0)))
                    }
                    ...
                    for product in data["products"]:
                        if product["id"] == product_id:
                            product.update({
                                ...
                                "cost": float(payload.get("costPrice", payload.get("cost", product.get("cost", 0)))),
                                "costPrice": float(payload.get("costPrice", product.get("costPrice", product.get("cost", 0)))),
                                "salesPrice": float(payload.get("salesPrice", product.get("salesPrice", product.get("rrp", 0))))
                            })
                            save_db(data)
            ...
            if path == "/api/invoices":
                if method == "GET":
                    ...
                    payload = self.parse_body()
                    currency = (payload.get("currency") or data["settings"].get("currency", "gbp")).lower()
                    invoice = {
